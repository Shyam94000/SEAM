...
class UserRoutes {
  ...
  initializeRoutes() {
    ...
    // Registration route with explicit POST method
    this.router.post('/register', this.upload.single('image'), async (req, res) => {
      try {
    ...
    // Authentication route with explicit POST method
    this.router.post('/authenticate', async (req, res) => {
      try {
        const { aadharNumber, descriptor } = req.body;
        // Validate inputs
        if (!aadharNumber || !descriptor) {
          return res.status(400).json({ error: 'Missing required fields' });
        }
        // Retrieve stored face descriptor and user data (including name, aadhar number, and image)
